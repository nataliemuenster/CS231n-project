...
OUTPUT_DIM = 49152 # Number of pixels in CIFAR10 (3*128*128)
OUTPUT_SIZE = 128 #from above output_dim
...
class Generator(nn.Module):
    ...
    def forward(self, input):
        ...
        return output.view(-1, 3, OUTPUT_SIZE, OUTPUT_SIZE)
...
def calc_gradient_penalty(netD, real_data, fake_data):
    ...
    alpha = alpha.expand(BATCH_SIZE, real_data.nelement()/BATCH_SIZE).contiguous().view(BATCH_SIZE, 3, OUTPUT_SIZE, OUTPUT_SIZE)
...
def generate_image(frame, netG):
    ...
    samples = netG(noisev)
    samples = samples.view(-1, 3, OUTPUT_SIZE, OUTPUT_SIZE)
...
def get_inception_score(G, ):
    ...
    all_samples = all_samples.reshape((-1, 3, OUTPUT_SIZE, OUTPUT_SIZE)).transpose(0, 2, 3, 1)
...
for iteration in xrange(ITERS):
    ...
    for i in xrange(CRITIC_ITERS):
        ...
        # train with real
        _data = _data.reshape(BATCH_SIZE, 3, OUTPUT_SIZE, OUTPUT_SIZE).transpose(0, 2, 3, 1)
    ...
    if iteration % 100 == 99:
        ...
        for images, _ in dev_gen():
            images = images.reshape(BATCH_SIZE, 3, OUTPUT_SIZE, OUTPUT_SIZE).transpose(0, 2, 3, 1)
