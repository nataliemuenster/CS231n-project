...
def visualize_image(tensor, name, label=None, env='main', w=250, h=250,
    ...
    _WINDOW_CACHE[title] = _vis(env).image(
        tensor.numpy(), win=_WINDOW_CACHE.get(title),
    ...
    if update_window_without_label:
        _WINDOW_CACHE[name] = _vis(env).image(
            tensor.numpy(), win=_WINDOW_CACHE.get(name),
...
def visualize_images(tensor, name, label=None, env='main', w=400, h=400,
    ...
    _WINDOW_CACHE[title] = _vis(env).images(
        tensor.numpy(), win=_WINDOW_CACHE.get(title), nrow=6,
        opts=dict(title=title, width=w, height=h)
    )

    # This is useful when you want to maintain the most recent images.
    if update_window_without_label:
        _WINDOW_CACHE[name] = _vis(env).images(
            tensor.numpy(), win=_WINDOW_CACHE.get(name), nrow=6,
            opts=dict(title=name, width=w, height=h)
        )
...
def visualize_kernel(kernel, name, label=None, env='main', w=250, h=250,
    ...
    _WINDOW_CACHE[title] = _vis(env).image(
        visualized, win=_WINDOW_CACHE.get(title),
        opts=dict(title=title, width=w, height=h)
    )
    ...
    if update_window_without_label:
        _WINDOW_CACHE[name] = _vis(env).image(
            visualized, win=_WINDOW_CACHE.get(name),
            opts=dict(title=name, width=w, height=h)
        )
...
def visualize_scalars(scalars, names, title, iteration, env='main'):
    ...
    if title in _WINDOW_CACHE:
        _vis(env).line(X=X, Y=Y, win=_WINDOW_CACHE[title], opts=options, update='append')
    else:
        _WINDOW_CACHE[title] = _vis(env).line(X=X, Y=Y, opts=options)
