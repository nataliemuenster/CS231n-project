...
train_dir = './dataset/dataset128/'
test_dir = './dataset/dataset128/'
train_set = []
val_set = []
...
#sampler=ChunkSampler(NUM_VAL, NUM_TRAIN)) #NUM_VAL, NUM_TRAIN??
...
def run_a_gan(D, G, D_solver, G_solver, discriminator_loss, generator_loss, show_every=20, 
              batch_size=32, noise_size=49152, num_epochs=100):
    """
    Train a GAN!
    ...
    for epoch in range(num_epochs):
        for x in loader_train:
            ...
            if (iter_count == 0 or iter_count % show_every == 0):
                im = fake_images.view(batch_size, 3, 128, 128)[0].transpose(0, 2)
                imgs_numpy = im.data.cpu().numpy()
#imgs_numpy = fake_images.data.cpu().numpy()
                save_images(imgs_numpy, 'dataset/out128/tmp_iter' + str(iter_count) + '.png')
#plt.show()
